...
class Descriptor:
    ...
    def __init__(self, protein_sequence: str):  # the function takes as input the protein sequence
        """	constructor """
    ...
    def get_aa_index1(self, name: str, path: str = '.'):
        """
    ...
    def get_aa_index23(self, name: str, path: str = '.'):
        """
    ...
    def get_lenght(self):
		...
		"""
        res = {}
        res['length'] = float(len(self.ProteinSequence.strip()))
        return res

    def get_charge(self, ph: float = 7.4, amide: bool = False):
        """
    ...
    def get_charge_density(self, ph: float = 7.0, amide: bool = False):
        """
    ...
    def get_formula(self, amide: bool = False):
        """
    ...
    def get_molar_extinction_coefficient(self):  # [reduced, oxidized] # with reduced cysteines / # with disulfid bridges
        """
    ...
    def get_paac(self, lamda: int = 10, weight: float = 0.05):
        """
    ...
    def get_paac_p(self, lamda: int = 10, weight: float = 0.05, AAP=None):
        """
    ...
    def get_apaac(self, lamda: int = 10, weight: float = 0.5):
        """
    ...
    def get_socn(self, maxlag: int = 45):
        """
    ...
    def get_socn_p(self, maxlag: int = 45, distancematrix=None):
        """
		Calculates the Sequence order coupling numbers  (retrieves 90 values by default) from pydpi
		:param maxlag: maximum lag. Smaller than length of the protein
		...
		:return: dictionary with the descriptors (90 descriptors)
		"""
        if maxlag > len(self.ProteinSequence): raise Exception(
    ...
    def get_qso(self, maxlag: int = 30, weight: float = 0.1):
        """
    ...
    def get_qso_p(self, maxlag: int = 30, weight: float = 0.1, distancematrix=None):
        """
		Calculates the Quasi sequence order  (retrieves 100 values by default) from pydpi
		:param maxlag: maximum lag. Smaller than length of the protein
		:param weight:
    ...
    def calculate_global(self, window: int = 1000, modality: str = 'max', scalename: str = 'Eisenberg'):
        """
    ...
    def calculate_profile(self, prof_type: str = 'uH', window: int = 7, scalename: str = 'Eisenberg'):
        """
    ...
    def calculate_arc(self, modality: str = "max", scalename: str = 'peparc'):
        """
    ...
    def calculate_autocorr(self, window: int = 7, scalename: str = 'Eisenberg'):
        """
    ...
    def calculate_crosscorr(self, window: int = 7, scalename: str = 'Eisenberg'):
        """
    ...
    def get_all_physicochemical(self, ph: float = 7, amide: bool = False):
        """
    ...
    def get_all_paac(self, lamda_paac: int = 10, weight_paac: float = 0.05, lamda_apaac: int = 10,
		...
		"""
        res = {}
        res.update(self.get_paac(lamda_paac, weight_paac))
        res.update(self.get_apaac(lamda_apaac, weight_apaac))
        return res
    ...
    def get_all_sequenceorder(self, maxlag_socn: int = 45, maxlag_qso: int = 30, weight_qso: float = 0.1):
        ...
        res = {}
        res.update(self.get_socn(maxlag_socn))
        res.update(self.get_qso(maxlag_qso, weight_qso))
        return res
    ...
    def adaptable(self, list_of_functions=None, ph: float = 7, amide: bool = False,
        ...
        for function in list_of_functions:
            if function == 1: res.update(self.get_lenght())
            if function == 2: res.update(self.get_charge(ph, amide))
            if function == 3: res.update(self.get_charge_density(ph, amide))
            if function == 4: res.update(self.get_formula(amide))
            if function == 5: res.update(self.get_bond())
            if function == 6: res.update(self.get_mw())
            if function == 7: res.update(self.get_gravy())
            if function == 8: res.update(self.get_aromacity())
            if function == 9: res.update(self.get_isoelectric_point())
            if function == 10: res.update(self.get_instability_index())
            if function == 11: res.update(self.get_sec_struct())
            if function == 12: res.update(self.get_molar_extinction_coefficient())
            if function == 13: res.update(self.get_flexibility())
            if function == 14: res.update(self.get_aliphatic_index())
            if function == 15: res.update(self.get_boman_index())
            if function == 16: res.update(self.get_hydrophobic_ratio())
            if function == 17: res.update(self.get_all_physicochemical(ph, amide))

            if function == 18: res.update(self.get_aa_comp())
            if function == 19: res.update(self.get_dp_comp())
            if function == 20: res.update(self.get_tp_comp())
            if function == 21: res.update(self.get_all_aac())

            if function == 22: res.update(self.get_paac(lamda_paac, weight_paac))
            if function == 23: res.update(self.get_paac_p(lamda_paac, weight_paac, AAP))

            if function == 24: res.update(self.get_apaac(lamda_apaac, weight_apaac))
            if function == 25: res.update(self.get_all_paac(lamda_paac, weight_paac, lamda_apaac, weight_apaac))

            if function == 26: res.update(self.get_moreau_broto_auto())
            if function == 27: res.update(self.get_moran_auto())
            if function == 28: res.update(self.get_geary_auto())
            if function == 29: res.update(self.get_all_correlation())

            if function == 30: res.update(self.get_ctd())

            if function == 31: res.update(self.get_conj_t())

            if function == 32: res.update(self.get_socn(maxlag_socn))
            if function == 33: res.update(self.get_socn_p(maxlag_socn, distancematrix))

            if function == 34: res.update(self.get_qso(maxlag_qso, weight_qso))
            if function == 35: res.update(self.get_qso_p(maxlag_qso, weight_qso, distancematrix))
            if function == 36: res.update(self.get_all_sequenceorder(maxlag_socn, maxlag_qso, weight_qso))

            # base class can take some time to run
            if function == 37: res.update(self.calculate_autocorr(window, scalename))
            if function == 38: res.update(self.calculate_crosscorr(window, scalename))
            if function == 39: res.update(self.calculate_moment(window, angle, modality, scalename))
            if function == 40: res.update(self.calculate_global(window, modality, scalename))
            if function == 41: res.update(self.calculate_profile(prof_type, window, scalename))
            if function == 42: res.update(self.calculate_arc(modality, scalename_arc))
            if function == 43: res.update(self.get_all_base_class(window, scalename, scalename_arc,angle, modality,
                                                                  prof_type))

            if function == 44:
                res.update(self.get_all(ph, amide, lamda_paac,
    ...
    #               lamda_apaac=10, weight_apaac=0.05, aap=None, maxlag_socn=45, maxlag_qso=30, weight_qso=0.1,
    #               distancematrix=None, window=7, scalename='Eisenberg', scalename_arc='peparc', angle=100,
    #               modality='max', prof_type='uH', bin_aa=False, bin_prop=False):
    #     """
# 	Function to calculate user selected descriptors
# 	:param list_of_functions: list of functions desired to calculate descriptors. Numeration in the descriptors guide.
# 	:param ph:parameters for geral descriptors
...
#     res = {}
#     print('lista')
#     all_funtions = [self.get_bin_aa(),self.get_bin_resi_prop(),self.get_lenght(),self.get_charge(ph, amide),
#                     self.get_charge_density(ph, amide),self.get_formula(amide),self.get_bond(),self.get_mw(),
#                     self.get_gravy(),self.get_aromacity(),self.get_isoelectric_point(),self.get_instability_index(),
#                     self.get_sec_struct(),self.get_molar_extinction_coefficient(),self.get_flexibility(),self.get_aliphatic_index(),
#                     self.get_boman_index(),self.get_hydrophobic_ratio(),self.get_all_physicochemical(ph, amide),
#                     self.get_aa_comp(),self.get_dp_comp(),self.get_tp_comp(),self.get_all_aac(),self.get_paac(lamda_paac, weight_paac),
#                     self.get_paac_p(lamda_paac, weight_paac, aap),self.get_apaac(lamda_apaac, weight_apaac),
#                     self.get_all_paac(lamda_paac, weight_paac, lamda_apaac, weight_apaac),self.get_moreau_broto_auto(),
#                     self.get_moran_auto(),self.get_geary_auto(),self.get_all_correlation(),self.get_ctd(),
#                     self.get_conj_t(),self.get_socn(maxlag_socn),self.get_socn_p(maxlag_socn, distancematrix),self.get_qso(maxlag_qso, weight_qso),
#                     self.get_qso_p(maxlag_qso, weight_qso, distancematrix),self.get_all_sequenceorder(maxlag_socn, maxlag_qso, weight_qso),
#                     self.calculate_autocorr(window, scalename),self.calculate_crosscorr(window, scalename),self.calculate_moment(window, angle, modality, scalename),
#                     self.calculate_global(window, modality, scalename),self.calculate_profile(prof_type, window, scalename),
#                     self.calculate_arc(modality, scalename_arc),self.get_all_base_class(window, scalename, scalename_arc),
#                     self.get_all(ph, amide, tricomp, lamda_paac,weight_paac, lamda_apaac, weight_apaac, maxlag_socn,maxlag_qso,weight_qso, window,
#                                  scalename,scalename_arc,angle, modality,prof_type, bin_aa, bin_prop)]
#     for function in list_of_functions:
#         res.update(all_funtions[function-1])
#     return res
