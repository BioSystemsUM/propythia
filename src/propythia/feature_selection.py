...
File containing a class used for feature selection. The FeatureSelection class aims to select features based on
supervised algorithms in order to improve estimators’ accuracy scores or to boost their performance on very
...
class FeatureSelection:
    ...
    def __init__(self, dataset, x_original, target, test_size=0.3):
        ...
        be provided. Test size is by default 0.3 but can be altered by user.
    ...
    def univariate(self,score_func=mutual_info_classif, mode='percentile', param=50,scaler = StandardScaler()):
        ...
        :param scaler:
            scaler function to use to datasets before apply univariate tests.
            It can be None or any function supported by SKlearn like StandardScaler()
        :param score_func:
            function that returns univariate scores and p-values (or only scores for SelectKBest and
            SelectPercentile)  ( for classification: chi2, f_classif, mutual_info_classif)
    ...
    def features_scores(self, x_original,scores, column_selected, all=False):
        ...
        :param column_selected: list containing the indexes of the selected columns (can be obtained by function univariate)
        :param all: to return all the features and scores or only the selected ones (by default)
        :return: dataframe containing the names of features and the scores of univariate tests by descending importance
        """

        if all:
            return pd.DataFrame(scores,index=x_original.columns,
    ...
    def recursive_feature_elimination(self,cross_validation=False,estimator=SVC(kernel="linear"),
        ...
        recursive feature elimination (RFE) is to select features by recursively considering smaller and smaller sets
        of features. First, the estimator is trained on the initial set of features and the importance of each feature
        is obtained either through a coef_ attribute or through a feature_importances_ attribute.
        ...
        :param verbose:
        :return: rfe fit, rfe transformed, original dataset with features selected, columns names and features ranking
        """
    ...
    def rfe_ranking(self,features_ranking):
        ...
        """
        return pd.DataFrame(features_ranking,index=self.dataset.columns,
                            columns=['features ranking']).sort_values(by=['features ranking'],ascending=True)

    def select_from_model_feature_elimination(self,model=LinearSVC(C=0.1, penalty="l1", dual=False),
        ...
        The features are considered unimportant and removed, if the corresponding coef_or feature_importances_values are
        ...
        if scaler:
            ...
            # self.Y_data, test_size=self.test_size, random_state=42)
        ...
        if 'Tree' in str(model):
            ...
            feature_importances_DF = pd.DataFrame(feature_importances,index = self.dataset.iloc[:,:-1].columns,
