...
# # data = remove_zeros(column='ec_single_label', data=data)
# data, fps_x_phys, columns = physchemical(data)
# fps_x_phys = fps_x_phys.drop(['ec_number2', 'ec_single_label'], axis=1) #9469 columns
...
# # model = KerasClassifier(build_fn=veltri_model, seq_len = seq_len, final_units = final_units, output_dim =128, batch_size=512)
#
...
data = hot90.dropna(subset=['Cross-reference (Pfam)']).reset_index()
data = get_ec_complete_more_than_x_samples(data, x=50, single_label=True)
